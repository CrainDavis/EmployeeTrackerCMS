  ...
  inquirer
    ...
    .then(function (answer) {
      if (answer.action === "View All Departments") {
        ...
        answer.action === "View Total Utilized Budget of a Department"
      ) {
...
var viewRelation = {
  ...
  employeesByDepartment: function (dep_name, cb) {
    connection.query(
      `SELECT employee.id, CONCAT(employee.first_name, ' ', employee.last_name) AS full_name, role.title, role.salary
        FROM department
  ...
  getFunctions.getDepartments(function (result) {
    ...
    inquirer
      ...
      .then(function (answers) {
        viewRelation.employeesByDepartment(answers.depChoice, function (
          result
        ) {
          console.log(
            "all employees in the " + answers.depChoice + " Department:\n"
          );
...
function deleteEmployee() {
  getFunctions.getEmployees(function (result) {
    ...
    inquirer
      ...
      .then(function (answers) {
        var employeeFirstName = answers.employeeName
          .split(" ")
          .slice(0, -1)
          .join(" ");
        var employeeLastName = answers.employeeName
          .split(" ")
          .slice(-1)
          .join(" ");
        connection.query(
